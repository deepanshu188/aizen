...
const comment = ref('')
const btnStyle = 'w-8 h-8 rounded-md border border-neutral-700 font-bold text-sm flex justify-center items-center hover:bg-neutral-700'
...
const postComment = async () => {
  try {
    const res = await addComment({ content: comment.value, post_id: id })
    console.log(res)
    comment.value = ''
  }
  catch { }
