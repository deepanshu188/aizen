<template>
  <div class="flex items-center my-2 cursor-pointer" @click="handleViewPost(post.id)">
    <div v-if="post.thumbnail_url && !thumbnailError" class="md:h-28 h-24 aspect-square"
      :class="{ 'blur-lg': post.nsfw && !settings.nsfw }">
      <NuxtImg :src="post.thumbnail_url" format="webp" class="w-full h-full rounded-md object-cover" loading="lazy"
        placeholder />
    </div>
