...
const { data, logoutUser } = useLoginUser();
const userInfo = computed(() => data.value?.user_data?.person_view?.person);
</script>

<template>
  <ul
    class="menu min-w-64 bg-base-200 sticky top-0 h-screen flex items-between max-sm:hidden p-4"
  >
    <p
    ...
    <template v-for="item in sideBarLinks" :key="item.name">
      <li
        v-if="item.private ? userInfo : true"
        class="my-1"
    ...
    <li v-if="userInfo" class="my-1">
      <div @click="logoutUser" class="gap-x-4 max-sm:px-0 max-sm:py-1">
